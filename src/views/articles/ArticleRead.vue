...
async function fetchArticle() {
  ...
  if (!slug.value) {
    loadError.value = 'No se encontró el artículo solicitado.'
    loading.value = false
    return
  }
  ...
  try {
    ...
    if (!data) {
      loadError.value = 'Este artículo no existe o todavía no está publicado.'
      return
    }

    article.value = data
    images.value = await fetchArticleImages(data.id)
  } catch (error) {
    console.error('Error al cargar artículo:', error)
    loadError.value = 'No se pudo cargar el artículo. Intenta nuevamente.'
  } finally {
    loading.value = false
  }
}
...
async function fetchArticleImages(postId) {
  ...
  try {
    ...
    return (data || [])
      .map((file) => {
        ...
        return {
          id: file.id,
          title: file.title || '',
          alt: file.alt_text || file.title || article.value?.title || '',
    ...
    console.warn('No se pudieron cargar imágenes del artículo:', error)
    return []
  }
}
...
function getSummary(articleValue) {
  return articleValue?.excerpt || articleValue?.subtitle || ''
}
...
function getReadingTime(articleValue) {
  const words = stripHtml(`${articleValue?.title || ''} ${articleValue?.excerpt || ''} ${articleValue?.content || ''}`)
    .trim()
    .split(/\s+/)
    .filter(Boolean)
    .length
  ...
  <main class="article-read">
    ...
    <section v-else-if="loadError" class="read-state read-state-error" role="alert">
      ...
      <div class="state-actions">
        <button type="button" class="read-button" @click="fetchArticle">Reintentar</button>
        <RouterLink to="/articulos" class="read-button read-button-light">Volver a artículos</RouterLink>
      </div>
    </section>
    ...
    <template v-else-if="article">
      <header class="article-header">
        <div class="read-shell">
          <RouterLink to="/articulos" class="back-link">
            <i class="bi bi-arrow-left" aria-hidden="true"></i>
            Artículos
          </RouterLink>

          <h1>{{ article.title || 'Artículo sin título' }}</h1>
      ...
      <section class="read-shell article-layout">
        <article class="article-content">
          ...
          <p v-if="getSummary(article)" class="article-summary">
            {{ getSummary(article) }}
          </p>
          ...
          <section v-if="galleryImages.length" class="article-gallery" aria-label="Galería del artículo">
            <h2>Galería</h2>
            <div class="gallery-grid">
              ...
              >
                <img :src="image.previewUrl" :alt="image.alt || article.title" loading="lazy">
              </a>
            </div>
          </section>
        </article>
        ...
        <aside class="article-sidebar">
          <section class="side-panel">
            ...
            <dl>
              <div>
                <dt>Publicado</dt>
                <dd>{{ formatDate(article.published_at || article.created_at) }}</dd>
              </div>
              <div>
                <dt>Lectura</dt>
                <dd>{{ getReadingTime(article) }}</dd>
              </div>
            </dl>
          </section>
